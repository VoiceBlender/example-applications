...
// the WebRTC stream and the segments light to the real speech level. Here it
// meters the INCOMING audio, which in this app is the interpreted voice — so it
// shows you the translation actually playing, not your own microphone. That
// makes it the fastest way to tell "the interpreter is speaking" from "the
// pipeline stalled".
//
// Shared verbatim with the push-to-talk example, save for the audio context it
// borrows.
//
...
window.Meter = (function () {
  ...
  function attach(stream) {
    stop();
    if (!stream) return;
    const actx = window.AudioCtx && AudioCtx.ctx();
    if (!actx) return;
    try {
